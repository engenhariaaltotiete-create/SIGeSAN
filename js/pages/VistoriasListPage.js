...
function VistoriasListPage({ navigate }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await VistoriasAPI.list({ q: query, status, page, pageSize: window.APP_CONFIG.ITEMS_PER_PAGE });
      setItems(res.data);
      setTotal(res.total);
      setTotalPages(res.totalPages);
    } catch (err) {
      window.toast.error('Erro ao listar vistorias: ' + err.message);
